...
<!-- varStatus 루프의 현재 상태를 알려주는 변수 이름
	...
	first		: 루프의 처음일때 true
	last		: 루프의 마지막일때 true-->
<!-- 일반 for문의 경우 current, index, var(i) 3가지 값이 동일 -->
<!-- 장바구니에서 많이 쓰임 -->
<table border="1">
...
<h2>1~100까지 정수중 홀수의 합</h2>
<c:set var="sum" value="0"/>
<c:forEach begin="1" end="100" var="num">
	<c:if test="${num mod 2 eq 1 }">
		 ${sum = sum+num }	<!-- 또는 <c:set var="sum" value="${sum + num}" />  -->
	</c:if>
</c:forEach>
...
%>
<table border="1">
<c:forEach items="<%=rgba %>" var="color" varStatus="col">
	<!-- 	index: 0부터 시작
			count: 1부터 시작(반복한 수만큼 증가) -->
	<c:set value='${loop.count mod 2 eq 1 ? "odd" : "" }' var="odd"/>
	odd : ${odd }
	<tr class="${odd }">
		<td>color: ${color }</td>
		<td>col.current : ${col.current }</td>
		<td>col.index: ${col.index }</td>
		<td>col.count: ${col.count }</td>
		<td>col.first: ${col.first }</td>
		<td>col.last: ${col.last }</td>
	</tr>
</c:forEach>
</table>
...
<%
	...
	//이렇게 하거나 아래 처럼 하거나 request.setAttribute("list", list);
%>
<!-- el표현언어를 통해 사용하기 위해서 
