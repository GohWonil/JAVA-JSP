...
<!--
	EL
		표현언어:	4가지 내장영역 속성의 값 출력할 때 사용
				값이 null인 경우에도 오류가 발생 하지 않음
		사용방법:	\${영역.속성} -> \${속성}
				\주석 처리를 안하면 자료 출력이 되서 처리함
				영역을 지정해 주지 않은 경우, 작은영역(page) 부터 
				큰 영역(application)까지 확인하여 값을 출력하기 때문에 속성의 이름이 중복되지 않는 경우라면
				영역을 지정하지 않아도 문제가 발생 하지 않는다.
